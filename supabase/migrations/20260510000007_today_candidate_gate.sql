-- 오늘의 투표 후보 신청 시 광고/이용권 게이트 도입
--
-- 변경 내용:
--   - register_vote 의 type='today_candidate' 분기에 광고/이용권 검증 추가
--     (기존: 1일 1건 cap 만 검증, 광고/이용권 무관)
--   - 새 정책: 후보 신청은 매번 무료이용권 1개 차감 OR 광고 시청 후 등록
--   - 'free pass not applicable to today_candidate' 에러 제거
--
-- 본문은 20260510000006 의 register_vote 와 동일 — today_candidate 분기만 normal 패턴 적용.
...
begin
  ...
  if p_type = 'normal' then
    ...
    -- ★ 후보 신청은 매번 광고 또는 무료이용권 필요 (정책 변경 — 20260510000007)
    if coalesce(p_use_free_pass, false) then
      update public.users
      set free_pass_balance = free_pass_balance - 1
      where id = v_uid
      ...
      raise exception 'ad or free pass required for today candidate' using errcode = 'P0004';
    end if;
  end if;
